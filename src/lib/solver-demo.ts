		...
		if (ironPlate) {
			...
			if (result.success && result.rootNode) {
				...
				result.rootNode.requiredInput.forEach((input) => {
					...
					console.log(
						`  - ${inputItem?.name || input.item}: ${input.amountPerMinute.toFixed(2)}/min`
					);
				});
			} else {
			...
			if (result.success && result.rootNode) {
				...
				function printInputTree(
					...
					inputs.forEach((input) => {
						const inputItem = data.items[input.item];
						console.log(
							`${indent}- ${inputItem?.name || input.item}: ${input.amountPerMinute.toFixed(2)}/min`
						);
						if (input.children) {
							input.children.forEach((child) => {
								console.log(
									`${indent}  ↳ Recipe: ${child.recipe.name} (${child.amount} buildings)`
								);
			...
			if (result.success && result.rootNode) {
				...
				function printTree(node: RecipeNode, depth = 0) {
					...
					node.requiredInput.forEach((input) => {
						const inputItem = data.items[input.item];
						console.log(
							`${indent}  Input: ${inputItem?.name || input.item} - ${input.amountPerMinute.toFixed(2)}/min`
						);
						if (input.children) {
