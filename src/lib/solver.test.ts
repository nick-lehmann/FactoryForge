...
describe('Solver', () => {
  ...
  it('should solve for iron plate production', () => {
    ...
    expect(result.rootNode?.building.name).toBe('Smelter');
    expect(result.rootNode?.amount).toBeGreaterThan(0);
    expect(result.rootNode?.requiredInput).toHaveLength(1);
    expect(result.rootNode?.requiredInput[0].item).toBe('Desc_OreIron_C');
  });
