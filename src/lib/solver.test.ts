...
describe('Solver', () => {
	...
	it('should solve for iron plate production', () => {
		...
		expect(result.rootNode.buildingCount).toEqual(1);
		expect(result.rootNode.requiredInputs).toHaveLength(1);

		// 30/min iron ingots
		const ingotsInput = result.rootNode.requiredInputs[0];
		...
		expect(ingotProducer.buildingCount).toBe(1); // We need only one smelter

		expect(ingotProducer.requiredInputs).toHaveLength(1);
		expect(ingotProducer.requiredInputs[0].item).toEqual('Desc_OreIron_C');
		expect(ingotProducer.requiredInputs[0].amountPerMinute).toEqual(30);
	});

	it('should solve for modular frame production', async () => {
		const result = solve(
			{
				item: mockData.items['Desc_ModularFrame_C'],
				amount: 10
			},
			mockData
		);
		...
		await expect(result).toMatchFileSnapshot('snapshots/modular_frames_10');
	});
